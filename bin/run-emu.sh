...
#while [ 1 ] ; do
  ./emulator -avd Pixel_5_API_33 -feature -Vulkan  -logcat-output /dev/stdout -logcat '*:i'  -camera-front webcam0  -camera-back webcam0 -partition-size 2047  -noaudio -no-boot-anim -no-snapshot-load  | egrep -v 'GnssLocationProvider|GnssHAL_GnssInterface' | tee emulator.log |  ack --passthru "ReactNative|AndroidRuntime|GoLog"
#done
